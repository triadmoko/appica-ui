...
- [x] `calendar` - Wave 2: bits-ui Calendar/RangeCalendar. `mode` → `type`. Values are `DateValue` from `@internationalized/date` (`CalendarDate`, `CalendarDateTime`, …), not native `Date`. Range is `{ from?, to? }`, mapped internally to bits-ui `{ start, end }`. Caption matches React (`dropdown` / `dropdown-months` / `dropdown-years` / `label`) via Appica `Select`. `required` maps to bits-ui `preventDeselect`. `month` / `onMonthChange` → `placeholder` / `onPlaceholderChange`. `disabled` Matcher → `isDateDisabled`.
