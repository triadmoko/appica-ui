<script lang="ts">
  ...
  export type ComboboxInputProps = WithoutChildrenOrChild<BitsCombobox.InputProps> & {
    start?: Snippet
    end?: Snippet
  }
...
>
  {#if start}
    <div data-slot="combobox-input-start" class="-ms-1 shrink-0">
      {@render start()}
    </div>
  {/if}
  <BitsCombobox.Input
  ...
  {/if}
  {#if end}
    <div data-slot="combobox-input-end" class="shrink-0">
      {@render end()}
    </div>
  {/if}
  {#if ctx.icon}
