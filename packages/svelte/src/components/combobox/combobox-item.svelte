...
<BitsCombobox.Item data-slot="combobox-item" {value} {label} {disabled} class={classes} {...asBitsAttrs(rest)}>
  {#snippet children({ selected })}
    <span class={cn('flex items-center text-start', ITEM_TEXT_SIZE[ctx.size])}>{@render itemLabel?.()}</span>
    <span data-slot="combobox-item-indicator" class="group/check text-foreground-intense shrink-0" data-selected={selected ? '' : undefined}>
      <svg
        data-icon="end"
