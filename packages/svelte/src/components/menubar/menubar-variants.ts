...
import type { MenubarSize } from './menubar-context'

export type MenubarSide = 'top' | 'right' | 'bottom' | 'left' | 'inline-end' | 'inline-start'
export type MenubarAlign = 'start' | 'center' | 'end'

export const POPUP_SIZE: Record<MenubarSize, string> = {
  sm: 'min-w-40 rounded-md',
  md: 'min-w-48 rounded-lg',
  lg: 'min-w-56 rounded-xl',
}

export const ICON_SIZE: Record<MenubarSize, string> = {
  sm: 'size-4',
  md: 'size-4.5',
  lg: 'size-5',
}

export const ITEM_TEXT: Record<MenubarSize, string> = {
  sm: 'gap-1',
  md: 'gap-1.5',
  lg: 'gap-1.5',
}

export const GROUP_LABEL_SIZE: Record<MenubarSize, string> = {
...
export const ITEM_ORIENTATION = 'vertical' as const
...
export function physicalSide(side: MenubarSide, dir: 'ltr' | 'rtl'): 'top' | 'right' | 'bottom' | 'left' {
  switch (side) {
    case 'top':
    case 'bottom':
    case 'left':
...
export function popupClassName(size: MenubarSize, className?: ClassValue) {
  return cn(
    'max-h-(--bits-menubar-content-available-height) w-(--bits-menubar-anchor-width) bg-background border-border-overlay flex flex-col border shadow-2xl outline-none',
    POPUP_SIZE[size],
