...
<BitsDropdownMenu.RadioItem data-slot="dropdown-menu-radio-item" {value} {disabled} class={classes} {...asBitsAttrs(rest)}>
  {#snippet children({ checked })}
    <span class={cn('flex items-center', ITEM_TEXT[ctx.size])}>{@render label?.()}</span>
    <span data-slot="dropdown-menu-radio-item-indicator" class="group/check text-foreground-intense shrink-0" data-checked={checked ? '' : undefined}>
      <svg
        data-icon="end"
