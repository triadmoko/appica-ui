<script lang="ts">
  ...
  const modal = $derived(ctx?.getModal() ?? false)
  const trapFocus = $derived(modal === true || modal === 'trap-focus')
  const preventScroll = $derived(modal === true)
  const labelledBy = $derived(ctx?.isTitled() ? ctx.titleId : undefined)
  const describedBy = $derived(ctx?.isDescribed() ? ctx.descriptionId : undefined)
...
<BitsPopover.Portal {...asBitsAttrs(portalProps ?? {})} to={container}>
  <BitsPopover.Content
    {side}
    ...
    dir={resolvedDir}
  >
    <div data-slot="popover-content" class={cardClasses}>
      {@render children?.()}
    </div>
    {#if arrow}
      <BitsPopover.Arrow width={26} height={18}>
        {#snippet child({ props })}
