export type CollectionGroupData<T = unknown> = {
  value: string
  items: readonly T[]
}

export function isGroupedItems(items: readonly unknown[]): items is CollectionGroupData[] {
  const first = items[0]
  return typeof first === 'object' && first !== null && Array.isArray((first as CollectionGroupData).items)
}
