...
import { ColorSwatch, type ColorSwatchProps } from './color-swatch'
import { parseColor } from '../../lib/color'

function renderSwatch(props: ColorSwatchProps) {
  return render(<ColorSwatch data-testid="swatch" {...props} />)
}
...
describe('ColorSwatch', () => {
  ...
  it('draws a translucent color over a checkerboard so the transparency reads', () => {
    ...
    // The tile size has to be its own declaration: `background-image` rejects the
    // `0 0 / size` shorthand and drops the whole rule.
  ...
  it('describes greys and the extremes without reaching for a hue', () => {
    ...
    rerender(<ColorSwatch data-testid="swatch" color="#ffffff" />)
    expect(swatch()).toHaveAccessibleName('white')

    rerender(<ColorSwatch data-testid="swatch" color="#808080" />)
    expect(swatch()).toHaveAccessibleName('gray')

    rerender(<ColorSwatch data-testid="swatch" color="rgba(0, 0, 0, 0)" />)
  ...
  it('sizes from a preset or an exact pixel number', () => {
    const { rerender } = renderSwatch({ color: '#3b82f6' })
    // The box is 1em, so the preset scale and a number both land on font-size.
    expect(swatch()).toHaveClass('text-[2.5rem]')
    expect(swatch()).toHaveClass('size-[1em]')

    rerender(<ColorSwatch data-testid="swatch" color="#3b82f6" size={20} />)
  ...
  it('takes a size class override on the root', () => {
    renderSwatch({ color: '#3b82f6', className: 'size-10' })
  ...
  it('rounds the corners by default and goes fully round on request', () => {
    ...
    rerender(<ColorSwatch data-testid="swatch" color="#3b82f6" shape="circle" />)
  ...
  it('drops the color when disabled, so the flat fill can show', () => {
    ...
    // An inline background would paint straight over the disabled class.
  ...
  it('renders children over the color', () => {
    renderSwatch({ color: '#3b82f6', children: <span data-testid="check">✓</span> })
